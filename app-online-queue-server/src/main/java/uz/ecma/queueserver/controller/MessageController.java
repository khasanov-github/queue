...
public class MessageController {
    ...
    @GetMapping("/complain")
    public HttpEntity<?> getComplains() {
        ApiResponse response = messageService.getComplains();
        return ResponseEntity.status(response.isSuccess() ? HttpStatus.ACCEPTED : HttpStatus.CONFLICT).body(response);
    }

    @PutMapping("/complain")
    public HttpEntity<?> deleteComplain(@RequestBody String name) {
            ApiResponse response = messageService.deleteComplain(name);
        return ResponseEntity.status(response.isSuccess() ? HttpStatus.ACCEPTED : HttpStatus.CONFLICT).body(response);
    }
