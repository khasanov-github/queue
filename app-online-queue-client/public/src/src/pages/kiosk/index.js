...
class Kiosk extends Component {
  ...
  componentDidMount() {
    ...
    const {directionId, delaying} = globalModel;
  ...
  render() {
    ...
    const {globalModel, queueModel} = this.props;
                    ...
                    <div className="w-100 bg-danger border-bottom">
                      <Row>
                        <Col className="col-md-1 text-white">
                          <h4 className="mt-4 pt-2 ml-3">#{i + 1}</h4>
                        </Col>
                        <Col className="col-md-6">
                          <div className="ml-3 text-white">
                            <h6 className="mt-2">Tel: {item.userPhoneNumber}</h6>
                            <p className="mt-n2">Navbat raqami: {item.queueNumber}</p>
                            <p>Qabul vaqti: {item.startTime}</p>
                          </div>
                        </Col>
                        <Col className="col-md-5">
                          <div className="text-white">
                            <h6 className="mt-2">Holati : {item.status}</h6>
                            <p className="mt-n1">Kechikish vaqti: {item.startTime}</p>
                            <div className="mb-2">
                              <MDBBtn color="primary" size="sm" onClick={() => toAccepted(item)}
                    ...
                    <div className=" mt-2 ml-4 w-100 text-white">
                      ...
                      <Row>
                        <Col className="offset-3 mb-3 mt-5">
                          <MDBBtn color="primary" disabled={isAccepted?false:true} onClick={toClosed}>Tugatish</MDBBtn>
                        </Col>
                      </Row>
                    </div>
                  </div>
                      ...
                      <div className="w-100 border-bottom">
                        <Row>
                            ...
                            <div className="ml-3 text-white">
                              ...
                              {(i + 1) == 1 ?
                                <div className="mb-2 kiosk-div">
                                  <MDBBtn className="mt-2" color="success" size="sm"
                          ...
                          <Col className="col-md-5">
                            <div className="text-white">
                              <h6 className="mt-2">Holati : {item.status}</h6>
                              <p className="mt-n1">Navbat vaqti: {item.startTime}</p>
                            </div>
                          </Col>
                        </Row>
                      </div>
